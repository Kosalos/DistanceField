...
class DeltaView: UIView {
    ...
    func initialize(_ iname:String) {
        name = iname
        vSize.x = bounds.width
        vSize.y = bounds.height
        vCenter.x = vSize.x/2
        vCenter.y = vSize.y/2
        setNeedsDisplay()
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        drawVLine(vCenter.x,0,bounds.height)
        drawHLine(0,bounds.width,vCenter.y)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for t in touches {
            ...
            deltaX =  Float( (pt.x - vCenter.x) / vSize.x / 10 )
            deltaY = -Float( (pt.y - vCenter.y) / vSize.y / 10 )
            touched = true
            setNeedsDisplay()
        }
    }
